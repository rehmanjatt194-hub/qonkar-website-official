<?php
// handlers/apply-process.php - Job Application Submission Handler
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['resume']) && $_FILES['resume']['error'] === UPLOAD_ERR_OK) {
        ...
        $resumeDir = dirname(__DIR__) . '/uploads/resumes/';
        if (!is_dir($resumeDir)) mkdir($resumeDir, 0777, true);
        $resumeRelPath = 'uploads/resumes/' . uniqid('resume_') . '.' . $ext;
        $resumePath = dirname(__DIR__) . '/' . $resumeRelPath;
