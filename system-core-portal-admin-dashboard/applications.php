...
<body>
    ...
    <main id="mainContent" class="flex-1 ml-64 text-white relative transition-all duration-300">
        ...
        <div class="p-8">
                ...
                <table id="applications_table" class="display cell-border stripe hover w-full">
                    <thead>
                        <tr>
                            ...
                            <th>Status</th>
                            <th>Resume</th>
                            <th>Transaction Slip</th> <!-- ✅ New Column -->
                            <th>Applied On</th>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        <?php
                        $result = $conn->query("SELECT ja.*, j.job_title FROM job_applications AS ja JOIN jobs AS j ON ja.job_id = j.job_id ORDER BY ja.id DESC;");
            ...
            <div class='flex flex-col items-center justify-center'>
                <span class='px-3 py-1 rounded text-white bg-yellow-600 mb-2'>Pending</span>
                <div class='flex gap-2'>
                    <button class='shortlist-btn bg-green-600 text-white px-3 py-1 rounded' data-id='{$row['id']}'>Shortlist</button>
                    <button class='reject-btn bg-red-600 text-white px-3 py-1 rounded' data-id='{$row['id']}'>Reject</button>
                </div>
            </div>
        ";
                            } elseif ($row['status'] === 'shortlist') {
                                $statusHTML = "<span class='px-3 py-1 rounded text-white bg-green-600'>{$statusText}</span>";
                            } elseif ($row['status'] === 'reject') {
                                $statusHTML = "<span class='px-3 py-1 rounded text-white bg-red-600'>{$statusText}</span>";
                            }

                            $resumeBtn = "<a href='../" . htmlspecialchars($row['resume']) . "' download  class='stop-row hover:bg-blue-600 text-white px-3 py-1 rounded'>📃</a>";

                            $transactionBtn = $row['transaction_slip']
                                ? "<button class='stop-row view-slip-btn bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded' data-slip='../" . htmlspecialchars($row['transaction_slip']) . "'>View Slip</button>"
                                : "<span class='text-gray-400 italic'>No Slip</span>";


                            echo "<tr class='app-row cursor-pointer' data-details='" . json_encode($row, JSON_HEX_APOS | JSON_HEX_QUOT) . "'>
        <td>{$row['id']}</td>
        ...
        <div id="slipModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 text-black">
            <div class="bg-white rounded shadow-lg w-11/12 md:w-2/3 lg:w-1/2 p-6 relative">
                <button id="closeSlipModal" class="absolute top-2 right-2 text-gray-600 hover:text-black">&times;</button>
                <h2 class="text-xl font-semibold mb-4">Transaction Slip</h2>
                <div id="slipContent" class="flex justify-center items-center">
                    <!-- Image or PDF will load here -->
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener("click", function(e) {
            // ✅ Transaction Slip Button Click
            if (e.target.classList.contains("view-slip-btn")) {
                e.stopPropagation(); // 🛑 Prevent row click modal
                const slipPath = e.target.getAttribute("data-slip");
                const slipContent = document.getElementById("slipContent");

                if (slipPath.toLowerCase().endsWith(".pdf")) {
                    slipContent.innerHTML = `<iframe src="${slipPath}" class="w-full h-[500px]"></iframe>`;
                } else {
                    slipContent.innerHTML = `<img src="${slipPath}" class="max-h-[500px] object-contain rounded shadow" alt="Transaction Slip">`;
                }
            ...
            // ✅ Close Transaction Slip Modal
            if (e.target.id === "closeSlipModal") {
                document.getElementById("slipModal").classList.add("hidden");
            }

            // ✅ Resume Button Click - Stop Row Click
            if (e.target.closest("a[download]")) {
                e.stopPropagation(); // 🛑 Prevent row click when downloading resume
            }
        });
        $(document).ready(function() {
            ...
            $(document).on("click", ".app-row", function(e) {
                if ($(e.target).closest(".stop-row").length) return; // 🛑 Don't open modal if stop-row clicked
